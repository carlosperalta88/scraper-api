...
describe('courts service', () => {
  ...
  it('should create one court', async () => {
    ...
    expect(court[0]).to.include({"name": "3º Juzgado de Letras de Iquique", "external_id": 11})
  })
  ...
  it('should get a court by name', async () => {
    const court = await CourtsService.search({ 'name': '3º Juzgado de Letras de Iquique'})
    expect(court).to.be.an('array')
